...
export function ZoneList({ zones, cameraName, editingZoneId, onEdit }: ZoneListProps) {
  const { deleteZone, toggleZone } = useZones()
      ...
      {zones.map((zone) => (
        ...
        >
          ...
          <div className="mt-3 flex flex-wrap gap-1">
            ...
            <Button variant="outline" size="sm" onClick={() => handleDelete(zone)}>
            ...
            >
              {zone.enabled ? (
                <ToggleRight className="h-4 w-4 text-text-secondary" />
              ) : (
