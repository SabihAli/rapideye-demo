...
flowchart TB
  subgraph inputs [StreamInputs]
    YT1[assets/camera_1.mp4]
    YT2[assets/camera_2.mp4]
    YT3[assets/camera_3.mp4]
    YT4[assets/camera_4.mp4]
  end

  subgraph server [DemoServer_FastAPI]
    Ingest[StreamManager_CV2]
...
**Hardcoded local files:** Ingest layer decodes files located inside the `assets/` folder (or RTSP streams) directly using OpenCV VideoCapture:
- Camera URLs default to `assets/camera_1.mp4` through `assets/camera_4.mp4`.
- Each camera runs an **independent decode thread** with its own ring buffer, reading at its target FPS.
- Supports resetting/restarting positions back to frame 0 via a `POST /api/demo/start` request.
...
| `POST /api/demo/start` | Start/Reset the demo feeds and clear alert histories |
