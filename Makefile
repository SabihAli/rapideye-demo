...
ifeq ($(OS),Windows_NT)
	...
	# InsightFace may pull the CPU onnxruntime package back in; remove it so
	# onnxruntime-gpu owns the import path, then reinstall the GPU runtime stack.
...
install-web:
	cd web && $(NODE_ENV_PREFIX) $(NPM_CMD) install
